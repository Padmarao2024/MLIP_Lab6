...
def test_data_split(feature_target_sample):
    ...
    # Unpack the 4 elements
    X_train, X_test, y_train, y_test = return_tuple

    # Verify none of the splits are empty
    assert len(X_train) > 0
    assert len(X_test) > 0
    assert len(y_train) > 0
    assert len(y_test) > 0

    # Verify total rows equal the original dataset rows
    assert len(X_train) + len(X_test) == len(feature_target_sample[0])

    # Verify features and labels are aligned in train and test sets
